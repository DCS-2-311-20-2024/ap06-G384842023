...
for (let r = 0; r < param.nRow; r += 1) {
  for (let c = 0; c < param.nCol; c += 1) {
    if (Math.random() < 0.4) { // 40%の確率で観客を配置
      const dummy = makeDummy("red"); // 赤色の観客を生成
      dummy.position.set(
        (param.w + param.gapX) * (c - (param.nCol - 1) / 2), // X位置
        (param.gapZ) * r - 2 + 0.8, // Y位置: 座席の高さ+0.8
        (param.d + param.gapZ) * (r + 5) // Z位置
      );
      scene.add(dummy); // 観客をシーンに追加
    }
  }
}
